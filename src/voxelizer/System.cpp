...
#undef max
    ...
    struct {
      ...
      float p1;
      glm::ivec3 dim;
    } cb = {.min = minP, .max = maxP, .dim = dimmensions};
    ...
    mHullPassVoxelOutput = focus::gContext->CreateShaderBuffer(
        nullptr, 8 * sizeof(u32), voxelOutputDesc);
  }
  ...
  focus::gContext->UnmapBufferPtr(mHullPassVertexOutput);
}
