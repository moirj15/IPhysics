...
struct BezierCurve {
    ...
    BezierCurve(const BezierCurve &b) :
        controlPoints(b.controlPoints), firstT(b.firstT), secondT(b.secondT), effectedPoints(b.effectedPoints)
    {
    }
    ...
    glm::vec3 EvaluateQuadraticCurve(f32 t)
    {
    ...
    glm::vec3 EvaluateCubicCurve(f32 t)
    {
...
struct Voxel {
    ...
    // TODO: clean-up
    [[nodiscard]] static glm::vec3 CalculateFrom3Points(const std::vector<glm::vec3> &points, f32 t)
    {
    ...
    [[nodiscard]] static glm::vec3 CalculateFrom4Points(const std::vector<glm::vec3> &points, f32 t)
    {
...
// TODO: Consider storing the mesh as interleaved data, might need a byte buffer?
struct Mesh {
    ...
    [[nodiscard]] glm::vec3 GetVertex(u32 i) const
    {
        assert(i < vertices.size() / 3);
        return {vertices[i * 3], vertices[(i * 3) + 1], vertices[(i * 3) + 2]};
    }

    [[nodiscard]] glm::vec3 GetNormal(u32 i) const
    {
    ...
    void SetVertex(u32 i, const glm::vec3 &v) { SetVec3(vertices, i, v); }
    void SetNormal(u32 i, const glm::vec3 &n) { SetVec3(normals, i, n); }
    [[nodiscard]] u32 VerticesSizeInBytes() const { return (u32)vertices.size() * sizeof(f32); }
    [[nodiscard]] u32 IndicesSizeInBytes() const { return (u32)indices.size() * sizeof(u32); }
     ...
     */
    [[nodiscard]] std::vector<f32> GetInterleved() const
    {
        std::vector<f32> ret;
...
private:
    static void SetVec3(std::vector<f32> &data, u32 i, const glm::vec3 &v)
    {
        data[i * 3] = v.x;
        data[(i * 3) + 1] = v.y;
        data[(i * 3) + 2] = v.z;
